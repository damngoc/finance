...
interface VndBankAccount {
  ...
  qrCode?: string  // URL QR code nếu có
}
...
const USDT_WALLETS: UsdtWallet[] = [
  {
    network: 'TRC20',
    address: 'TXyz1234567890ABCDefghijklmnopqrstu',
    note: 'Chỉ gửi USDT trên mạng TRON (TRC20)',
  },
  {
    network: 'ERC20',
    address: '0xAbCd1234567890EfGhIjKlMnOpQrStUvWxYz1234',
    note: 'Chỉ gửi USDT trên mạng Ethereum (ERC20)',
  },
]
    ...
    <div className="space-y-4">
      ...
      <div className="bg-slate-800/70 border border-slate-700 rounded-2xl overflow-hidden">
        ...
        <div className="divide-y divide-slate-700/50">
          {[
            { label: 'Tên ngân hàng',     value: account.bankName,      mono: false },
            { label: 'Tên chủ tài khoản', value: account.accountName,   mono: false },
            { label: 'Số tài khoản',      value: account.accountNumber, mono: true  },
          ].map(({ label, value, mono }) => (
            <div key={label} className="px-4 py-3.5 flex items-center justify-between gap-3">
              <div className="min-w-0">
                <p className="text-xs text-slate-400 mb-0.5">{label}</p>
                <p className={`text-white text-sm font-semibold ${mono ? 'font-mono tracking-wider' : ''}`}>
                  {value}
                </p>
              </div>
      ...
      {/* ── Khối 2: QR code (riêng biệt) ── */}
      {account.qrCode && (
        <div className="bg-slate-800/70 border border-slate-700 rounded-2xl overflow-hidden">
          <div className="px-4 py-3 bg-slate-800 border-b border-slate-700 flex items-center gap-2">
            <span className="text-base">📱</span>
            <span className="text-white font-semibold text-sm">QR Code chuyển khoản</span>
          </div>
          <div className="p-5 flex flex-col items-center">
            ...
            <p className="text-xs text-slate-400 mt-3">Quét mã để chuyển khoản nhanh</p>
          </div>
        </div>
      )}
      ...
      <div className="flex gap-2.5 px-3.5 py-3 bg-amber-500/8 border border-amber-500/25 rounded-xl">
        ...
        <p className="text-xs text-amber-300/80 leading-relaxed">
          Vui lòng chuyển khoản đúng <strong className="text-amber-300">số tài khoản</strong> và <strong className="text-amber-300">tên ngân hàng</strong>. Ghi chú nội dung chuyển khoản là <strong className="text-amber-300">tên đăng nhập</strong> của bạn.
        </p>
      </div>
    </div>
  )
      ...
      <div className={`bg-white border rounded-2xl overflow-hidden ${cfg.border}`}>
        ...
        <div className="px-4 py-4">
          ...
          <div className={`p-3 bg-slate-900 border ${cfg.border} rounded-xl`}>
            <p className="font-mono text-xs text-white break-all leading-relaxed tracking-wide">
              {wallet.address}
            </p>
          </div>
...
const ClientDepositPage: React.FC = () => {
  ...
  const validate = () => {
    ...
    if (currency === 'VND' && num < 50_000) {
      setAmountError('Số tiền tối thiểu là 50.000đ')
      return false
    }
    if (currency === 'USDT' && num < 5) {
      setAmountError('Số tiền tối thiểu là 5 USDT')
      return false
    }
    setAmountError('')
    return true
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        <button onClick={() => navigate(-1)} className="w-8 h-8 flex items-center justify-center rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600 transition-colors text-sm flex-shrink-0">
          ←
        </button>
        <div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="p-4 grid grid-cols-2 gap-3">
          ...
          >
            {currency === 'VND' && (
              <span className="absolute top-2 right-2 w-5 h-5 bg-violet-500 rounded-full flex items-center justify-center text-white text-xs">✓</span>
            )}
            <span className="text-3xl">🇻🇳</span>
            <div className="text-center">
              <p className={`font-bold text-sm ${currency === 'VND' ? 'text-violet-700' : 'text-slate-700'}`}>VND</p>
              <p className="text-xs text-slate-500">Tiền Việt Nam</p>
            </div>
          </button>
          ...
          >
            {currency === 'USDT' && (
              <span className="absolute top-2 right-2 w-5 h-5 bg-teal-500 rounded-full flex items-center justify-center text-white text-xs">✓</span>
            )}
            <span className="text-3xl">💵</span>
            <div className="text-center">
              <p className={`font-bold text-sm ${currency === 'USDT' ? 'text-teal-700' : 'text-slate-700'}`}>USDT</p>
        ...
        <div className="p-4 space-y-3">
          ...
          {amount && parseFloat(amount) > 0 && (
            <div className="flex items-center justify-between px-3.5 py-3 bg-slate-50 rounded-xl border border-slate-200">
              ...
              <span className={`text-base font-bold ${currency === 'VND' ? 'text-violet-600' : 'text-teal-600'}`}>
                {fmtAmount()}
              </span>
            </div>
          )}
