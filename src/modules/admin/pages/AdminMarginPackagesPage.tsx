...
import {
  adminMarginPackageService,
  MarginPackage,
  CreateMarginPackageDto,
} from '../services/adminMarginPackageService'
...
interface AddModalProps {
  isOpen: boolean
  onClose: () => void
  onSubmit: (dto: CreateMarginPackageDto) => Promise<void>
  isLoading: boolean
}

const AddModal: React.FC<AddModalProps> = ({ isOpen, onClose, onSubmit, isLoading }) => {
  const [name, setName] = useState('')
  const [selectedPreset, setSelectedPreset] = useState('30 ngày')
  ...
  const resolvedDays = isCustom ? parseInt(customDays) || 0 : preset?.days || 0
  const resolvedDuration = isCustom
    ? customDays ? `${customDays} ngày` : ''
    : selectedPreset

  const validate = () => {
    const e: Record<string, string> = {}
    if (!name.trim()) e.name = 'Vui lòng nhập tên gói'
    if (isCustom && (!customDays || parseInt(customDays) <= 0))
      e.duration = 'Vui lòng nhập số ngày hợp lệ'
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-md shadow-2xl">
        ...
        <div className="flex items-center gap-3 px-6 py-4 border-b border-gray-800">
          <div className="w-9 h-9 rounded-xl bg-teal-600/20 flex items-center justify-center text-xl flex-shrink-0">🏦</div>
          <div>
            <h2 className="text-white font-semibold">Thêm gói ký quỹ</h2>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="VD: Gói Vàng 90 ngày"
              className={`w-full px-3 py-2 rounded-xl text-sm bg-gray-800 border transition-colors focus:outline-none focus:ring-2 text-gray-200 placeholder-gray-600 ${
                errors.name ? 'border-red-500 focus:ring-red-500/30' : 'border-gray-700 focus:ring-teal-500/30 focus:border-teal-500'
              }`}
            />
            ...
            {isCustom && (
              <div className="relative mt-2">
                <input
                  type="number"
                  value={customDays}
                  onChange={e => { setCustomDays(e.target.value); setErrors(v => ({ ...v, duration: '' })) }}
                  placeholder="Nhập số ngày..."
                  min={1}
                  className={`w-full px-3 py-2 rounded-xl text-sm bg-gray-800 border transition-colors focus:outline-none focus:ring-2 text-gray-200 placeholder-gray-600 ${
                    errors.duration ? 'border-red-500 focus:ring-red-500/30' : 'border-gray-700 focus:ring-teal-500/30 focus:border-teal-500'
                  }`}
                />
            ...
            <div className="relative">
              <input
                ...
                className={`w-full px-3 py-2 pr-10 rounded-xl text-sm bg-gray-800 border transition-colors focus:outline-none focus:ring-2 text-gray-200 placeholder-gray-600 ${
                  errors.profit ? 'border-red-500 focus:ring-red-500/30' : 'border-gray-700 focus:ring-teal-500/30 focus:border-teal-500'
                }`}
              />
              <span className="absolute right-3 top-1/2 -translate-y-1/2 text-teal-400 font-bold text-sm">%</span>
            </div>
            {errors.profit && <p className="mt-1 text-xs text-red-400">⚠ {errors.profit}</p>}
            {profitPercent && parseFloat(profitPercent) > 0 && resolvedDays > 0 && (
              <div className="mt-2 px-3 py-2 bg-teal-500/5 border border-teal-500/20 rounded-xl space-y-1">
                <p className="text-xs text-gray-400">
                  Ký quỹ <span className="text-white font-medium">10.000.000đ</span> trong{' '}
                  <span className="text-teal-400 font-medium">{resolvedDuration}</span>
                </p>
                <p className="text-xs text-gray-400">
                  → Nhận về{' '}
                  <span className="text-teal-300 font-semibold">
                    {new Intl.NumberFormat('vi-VN').format(10000000 + Math.round(10000000 * parseFloat(profitPercent) / 100))}đ
                  </span>
                  {' '}(lãi <span className="text-teal-400">+{new Intl.NumberFormat('vi-VN').format(Math.round(10000000 * parseFloat(profitPercent) / 100))}đ</span>)
                </p>
              </div>
            )}
          </div>
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button type="submit" disabled={isLoading}
              className="flex-1 py-2.5 bg-teal-600 hover:bg-teal-500 text-white rounded-xl text-sm font-medium transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
              {isLoading
      ...
      <div className="relative bg-gray-900 border border-gray-700 rounded-2xl p-6 max-w-sm w-full shadow-2xl">
        <div className="text-center mb-5">
          ...
          <p className="text-gray-400 text-sm mt-2">
            Bạn có chắc muốn xóa gói <strong className="text-white">{pkg.name}</strong>?<br />
...
// ─── Main page ────────────────────────────────────────────────────
const AdminMarginPackagesPage: React.FC = () => {
  const [packages, setPackages] = useState<MarginPackage[]>([])
  const [loading, setLoading] = useState(true)
  const [addOpen, setAddOpen] = useState(false)
  const [deleteTarget, setDeleteTarget] = useState<MarginPackage | null>(null)
  ...
  const loadPackages = async () => {
    setLoading(true)
    try {
      const data = await adminMarginPackageService.getAll()
      setPackages(data)
    } catch {
  ...
  const handleCreate = async (dto: CreateMarginPackageDto) => {
    setSaving(true)
    try {
      const created = await adminMarginPackageService.create(dto)
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true)
    try {
      await adminMarginPackageService.delete(deleteTarget.id)
  ...
  const profitColor = (pct: number) => {
    if (pct >= 30) return 'text-blue-300'
    if (pct >= 20) return 'text-purple-400'
    if (pct >= 15) return 'text-yellow-400'
    if (pct >= 10) return 'text-emerald-400'
    if (pct >= 5)  return 'text-teal-400'
    return 'text-gray-300'
  }

  const profitBarColor = (pct: number) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white">🏦 Quản lý Gói Ký Quỹ</h1>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
        ...
        {!loading && packages.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <h3 className="text-gray-300 font-semibold text-lg">Chưa có gói ký quỹ</h3>
            <p className="text-gray-600 text-sm mt-1 mb-5">Bắt đầu bằng cách thêm gói đầu tiên</p>
            <button onClick={() => setAddOpen(true)} className="px-4 py-2 bg-teal-600 hover:bg-teal-500 text-white rounded-xl text-sm font-medium transition-colors">
              + Thêm gói đầu tiên
            </button>
          </div>
        )}
        ...
        {!loading && packages.length > 0 && (
          <div className="overflow-x-auto">
              ...
              <thead>
                <tr className="border-b border-gray-800">
                  {['STT', 'Tên Gói', 'Thời gian', '% Lợi nhuận', 'Ngày tạo', 'Thao tác'].map(h => (
                    <th key={h} className="text-left text-xs font-medium text-gray-500 uppercase tracking-wider px-5 py-4 whitespace-nowrap">
                      {h}
                    </th>
                  ))}
                </tr>
                  ...
                  <tr key={pkg.id} className="hover:bg-gray-800/40 transition-colors group">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-teal-500/20 to-emerald-600/20 border border-teal-500/20 flex items-center justify-center text-base flex-shrink-0">
                          🏦
                        </div>
                        <div>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        <span className="inline-flex items-center px-2.5 py-1 bg-teal-500/10 text-teal-400 border border-teal-500/20 rounded-lg text-xs font-mono font-bold">
                          {pkg.durationDays}d
                        </span>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex-1 max-w-[80px]">
                          <div className="h-1.5 bg-gray-800 rounded-full overflow-hidden">
                            <div
                              className={`h-full rounded-full ${profitBarColor(pkg.profitPercent)}`}
                              style={{ width: `${Math.min(100, (pkg.profitPercent / 40) * 100)}%` }}
                            />
                          </div>
                        </div>
                      </div>
            ...
            <div className="px-5 py-3 border-t border-gray-800 flex items-center justify-between">
              <span className="text-xs text-gray-600">{packages.length} gói ký quỹ</span>
              <div className="flex items-center gap-4 text-xs text-gray-600">
                <span>LN thấp nhất: <span className="text-teal-400 font-medium">{Math.min(...packages.map(p => p.profitPercent))}%</span></span>
                <span>LN cao nhất: <span className="text-blue-300 font-medium">{Math.max(...packages.map(p => p.profitPercent))}%</span></span>
              </div>
            </div>
          </div>
        )}
      ...
      <AddModal isOpen={addOpen} onClose={() => setAddOpen(false)} onSubmit={handleCreate} isLoading={saving} />
      <DeleteModal pkg={deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={handleDelete} isLoading={deleting} />
    </div>
  )
}

export default AdminMarginPackagesPage